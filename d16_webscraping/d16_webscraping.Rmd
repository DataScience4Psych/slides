...
- Parse HTML documents using rvest functions
- Extract data from websites using CSS selectors and SelectorGadget
- Transform scraped text into structured data frames
- Apply ethical considerations and check robots.txt before scraping
...
```{r echo=FALSE}
# In case the website is down, we have a backup copy of the page in the `backup` folder
page <- tryCatch(read_html("https://www.imdb.com/chart/top/"), error = function(e) {
  read_html( "backup//IMDb Top 250 movies.html")
})
page
```
...
page %>% 
  ...
  html_text() %>% # contains year
  str_subset(., "[12][09][0-9][0-9]") # filter for nodes that contain "4 digit year starting with 19 or 20"
```
]
.pull-right[
```{r echo=FALSE, out.width="100%"}
...
In previous years, we've need to go from `"(1994)"` to `1994`:
